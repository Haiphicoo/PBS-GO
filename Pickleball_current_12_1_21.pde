...
//Animation stuff
Animation pickler; //blue player
Animation greenPickler;
Animation redPickler; 


// make an array of puncher Images
// makes the array have 2 slots
PImage[] picklerImages = new PImage[2];
PImage[] greenPicklerImages = new PImage[2];
PImage[] redPicklerImages = new PImage[2];


void setup() {
  size(1200, 600);

  backgroundMusic = new SoundFile(this, "WiiSportsTheme.wav");
  paddleSound = new SoundFile(this, "ballSound.wav");
  ...
  instructionScreenImg = loadImage("instructions.JPG");
  instructionScreenImg.resize(1200, 600);
  ...
  // automatic player
  a1 = new Automatic(width/4, 40, color(255, 0, 0), 50, height - 40, winningScore, 0.9, 0.92);
  //ball
  ...
  //green
  for (int i=0; i<greenPicklerImages.length; i++) {
    greenPicklerImages[i] = loadImage("greenPlayer"+i+".png");
  }

  greenPickler = new Animation(greenPicklerImages, .2, 1);

  //red
  for (int i=0; i<redPicklerImages.length; i++) {
    redPicklerImages[i] = loadImage("redPlayer"+i+".png");
  }

  redPickler = new Animation(redPicklerImages, .2, 1);
...
void draw() {
  ...
  case 1:
    ...
    pickler.display(p5.x + 20, p5.y);
    //p5.render();
    p5.moveLeft();
    p5.moveRight();
    p5.scoring(b1);

    redPickler.display(int (a1.x), int(a1.y));
    //a1.render();
    a1.ease(b1);
    a1.hit(b1);
    a1.scoring(b1);
  ...
  case 2:
    ...
    pickler.display(p1.x + 20, p1.y);
    //p1.render();
    p1.moveLeft1();
    p1.moveRight1();
    p1.scoring(b2);

    greenPickler.display(p2.x + 20, p2.y);
    //p2.render();
    p2.moveLeft1();
    p2.moveRight1();
    p2.scoring(b2); 

    pickler.display(p3.x + 20, p3.y);
    //p3.render();
    p3.moveLeft2();
    p3.moveRight2();
    p3.scoring(b3);

    greenPickler.display(p4.x + 20, p4.y);
    //p4.render();
    p4.moveLeft2();
    p4.moveRight2();
    p4.scoring(b3);
  ...
  case 4:
    // instructions
    image(instructionScreenImg, 0, 0);
